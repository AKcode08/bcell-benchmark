...
def usage() -> None:
    print(
        "Usage: python scripts/05_discotope_benchmark.py <ORG1> [<ORG2> ...]",
        file=sys.stderr,
    )
    sys.exit(1)
...
def benchmark_one_org(org: str) -> pd.DataFrame | None:
    ...
    pred_path = Path(f"outputs/discotope3/{org}/linear/discotope3_indexed.csv")
    gt_path = Path(f"iedb/{org}/linear/linear_ground_truth.csv")
    out_path = Path(f"tables/{org}/linear/{org}_discotope3_results.csv")

    if not pred_path.exists():
        print(f"Skipping {org}: predictions not found", file=sys.stderr)
        return None

    if not gt_path.exists():
        print(f"Skipping {org}: ground truth not found", file=sys.stderr)
        return None
    ...
    if len(eval_df) > 0:
        ...
        by_protein_out = Path(
            f"tables/{org}/linear/{org}_discotope3_by_protein.csv"
        )
...
def main():
    ...
    if all_results:
        ...
        out = Path("tables/discotope3_linear_all_orgs_results.csv")
