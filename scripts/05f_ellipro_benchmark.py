...
def usage():
    print(
        "Usage: python scripts/06_ellipro_benchmark.py <ORG1> [<ORG2> ...]",
        file=sys.stderr
    )
    sys.exit(1)
...
def benchmark_one_org(org):
    ...
    pred_path = Path(f"outputs/ellipro/{org}/linear/ellipro_indexed.csv")
    gt_path = Path(f"iedb/{org}/linear/linear_ground_truth.csv")

    out_path = Path(f"tables/{org}/linear/{org}_ellipro_results.csv")
    ...
    if len(eval_df) > 0:
        ...
        by_protein_out = Path(
            f"tables/{org}/linear/{org}_ellipro_by_protein.csv"
        )
...
def main():
    ...
    if all_results:
        ...
        out = Path("tables/ellipro_linear_all_orgs_results.csv")
