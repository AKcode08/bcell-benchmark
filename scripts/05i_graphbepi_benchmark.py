...
def usage():
    print(
        "Usage: python scripts/07_graphbepi_benchmark.py <ORG1> [<ORG2> ...]",
        file=sys.stderr
    )
    sys.exit(1)
...
def benchmark_one_org(org):
    ...
    pred_path = Path(
        f"outputs/graphbepi/{org}/linear/graphbepi_indexed.csv"
    )

    gt_path = Path(
        f"iedb/{org}/linear/linear_ground_truth.csv"
    )

    out_path = Path(
        f"tables/{org}/linear/{org}_graphbepi_results.csv"
    )
    ...
    # =====================================================
    # LOOP THROUGH GROUND TRUTH EPITOPES
    # =====================================================
    ...
    for _, r in gt.iterrows():
        ...
        predicted_hits = int(
            (sub["raw_score"] >= THRESH).sum()
        )

        recall = (
            predicted_hits / ep_len
            if ep_len > 0 else 0.0
        )
...
def main():
    ...
    if all_results:
        ...
        out = Path(
            "tables/graphbepi_linear_all_orgs_results.csv"
        )
