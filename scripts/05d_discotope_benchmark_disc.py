...
def usage() -> None:
    print(
        "Usage: python scripts/05d_discotope_conformational_benchmark.py <ORG1> [<ORG2> ...]",
        file=sys.stderr,
    )
    sys.exit(1)
...
def benchmark_one_org(org: str) -> pd.DataFrame | None:

    group = get_group(org)

    pred_path = Path(
        f"outputs/discotope3/{org}/conformational/discotope3_indexed.csv"
    )
    ...
    out_path = Path(
        f"tables/{org}/conformational/{org}_discotope3_results.csv"
    )
    ...
    pred_pos = (
        pred.loc[pred["calibrated_score"] >= THRESH, ["uniprot", "res_index"]]
    ...
    if len(eval_df) > 0:
        ...
        by_protein_out = Path(
            f"tables/{org}/conformational/{org}_discotope3_by_protein.csv"
        )
...
def main():
    ...
    if all_results:
        ...
        combined_out = Path(
            "tables/discotope3_conformational_all_orgs.csv"
        )
