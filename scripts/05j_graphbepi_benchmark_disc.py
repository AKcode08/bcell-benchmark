...
def usage():
    print(
        "Usage: python scripts/07d_graphbepi_conformational_benchmark.py <ORG1> [<ORG2> ...]",
        file=sys.stderr
    )
    sys.exit(1)
...
def benchmark_one_org(org):
    ...
    pred_path = Path(
        f"outputs/graphbepi/{org}/conformational/graphbepi_indexed.csv"
    )
    ...
    out_path = Path(
        f"tables/{org}/conformational/{org}_graphbepi_results.csv"
    )
    ...
    if not required_pred_cols.issubset(pred.columns):
        ...
        raise SystemExit(
            f"{org}: pred file missing columns: {missing}"
        )
    ...
    if not required_gt_cols.issubset(gt.columns):
        ...
        raise SystemExit(
            f"{org}: gt file missing columns: {missing}"
        )
    ...
    for _, r in gt.iterrows():
        ...
        if n_valid == 0:
            ...
            rows.append({
                ...
                "reason": (
                    f"all_true_residues_out_of_range"
                    f"(max_true={max(true_set)},"
                    f"pred_len={int(max_i)})"
                ),
            })
        ...
        partially_out_of_range = (
            n_valid < len(true_set)
        )
    ...
    if len(eval_df) > 0:
        ...
        by_protein_out = Path(
            f"tables/{org}/conformational/"
            f"{org}_graphbepi_by_protein.csv"
        )
...
def main():
    ...
    if all_results:
        ...
        out = Path(
            "tables/graphbepi_conformational_all_orgs.csv"
        )
